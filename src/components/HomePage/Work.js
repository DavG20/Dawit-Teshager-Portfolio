...
function Work() {
  ...
  return (
    <div
      id="work"
      className="text-stone-300 w-full h-auto md:min-h-screen grow grid grid-cols-12 gap-4 justify-start md:py-8 md:mb-8 md:pt-24 md:px-14"
    >
            ...
            return (
              <div key={work.id} className="flex flex-col w-full">
                ...
                >
                  ...
                  {/* Background Overlay */}
                  <div className="h-full w-full justify-end bg-gradient-to-t from-black-50 to-black-0 flex flex-col absolute top-0 left-0 z-30"></div>

                  {/* Top Right Button */}
                  <div className="flex items-center absolute top-0 right-0 z-40 p-4 md:p-6 gap-2 group">
                    <button
                      className="flex group items-center gap-4 px-4 py-2 bg-white text-sm md:text-md text-custom-dark font-space-grotesk font-bold"
                      onClick={() => {
                        setShowWork(work);
                      }}
                    >
                  ...
                  <div className="flex flex-col justify-end w-full h-full absolute top-0 right-0 z-30 gap-2 group">
                    <div className="flex flex-col gap-1 bg-white p-4 md:p-6 group-hover:translate-y-4 duration-200">
                      ...
                      <div className="flex gap-4 duration-200 mt-6 justify-end">
                        <a
                          href={work.source}
                          className="group hover:scale-110 flex justify-end items-center gap-2 duration-200"
                        >
                        ...
                        <a
                          href={work.link}
                          className="flex hover:scale-125 justify-end items-center gap-2 duration-200"
                        >
      ...
      {showWork && (
        <DetailsPage onClose={() => setShowWork()} workData={showWork} />
      )}
    </div>
  );
}
